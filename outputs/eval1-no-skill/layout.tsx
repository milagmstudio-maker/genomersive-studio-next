...
// BackgroundFX の代わりに SSR-safe な ShaderCanvas を使う
  ...
  return (
    ...
    >
      <body className="bg-noise relative min-h-full">
        <Splash />
        {/*
          ShaderCanvas は next/dynamic({ ssr: false }) でラップされているため
          Cloudflare Workers（サーバー）では何もレンダリングせず、
          クライアント側でのみ <Canvas> が mount される。
