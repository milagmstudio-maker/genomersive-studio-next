...
| No. | コンポーネント | 見出し・コピー |
|-----|----------------|----------------|
| 001 | `Hero` | 肩書き「SOUND ENGINEER / DIRECTOR / PRODUCER」（経歴の順）→ 名前（Archivo 極太の2段。背景の波は全ページ網点で見せる＝`AmbientVideo.tsx`（明るさの差はページごとの幕で付ける）。肩書きは明るい網点に重なるので後ろに暗がりを敷き、「料金を見る」も中を暗く塗っている）→「音を整え、活動の次の一歩まで。」→「SOUND STUDIO FOR EVERY VOICE／声と音で活動する、すべての人へ」→ ボタン「実績を聴く」（白塗り）「料金を見る」（枠線） |
